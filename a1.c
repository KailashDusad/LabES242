...
void generate_splits_recursive(const char *source, const char *dictionary[], int nwords, char buf[], int buf_idx, void *data, void (*process_split)(char buf[], void *data)) {
    if (*source == '\0') {
        buf[buf_idx] = '\0';
    ...
    for (int i = 0; i < nwords; i++) {
        ...
        if (strncmp(source, word, word_len) == 0 ) {
            strcpy(buf + buf_idx, word);
